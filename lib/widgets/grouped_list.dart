...
class GroupedListView<T, E> extends StatefulWidget {
  ...
  final Widget Function(List<T> elements)? groupHeaderBuilder;
  ...
  final Widget Function(List<T> element)? groupStickyHeaderBuilder;
...
class _GroupedListViewState<T, E> extends State<GroupedListView<T, E>> {
      ...
      if (index == hiddenIndex) {
        ...
        return Opacity(
          ...
          child: _buildGroupSeparator(
              _sortedElements.sublist(actualIndex, endIndex)),
        );
      }
  ...
  /// Returns the group header [Widget] for an [elements] based on
  /// [widget.groupHeaderBuilder] or if null on the
  /// [widget.groupSeparatorBuilder].
  Widget _buildGroupSeparator(List<T> elements) {
    if (widget.groupHeaderBuilder == null) {
      return widget.groupSeparatorBuilder!(widget.groupBy(elements.first));
    }
    return widget.groupHeaderBuilder!(elements);
  }

  Widget _buildFixedGroupHeader(List<T> elements) {
    if (widget.groupStickyHeaderBuilder == null) {
      return _buildGroupSeparator(elements);
    }
    return widget.groupStickyHeaderBuilder!(elements);
  }
